...
import 'package:app_produtividade/pages/Notes%20App/drawing_canvas/models/drawing_mode.dart';
import 'package:app_produtividade/pages/Notes%20App/drawing_canvas/models/sketch.dart';
...
class DrawingCanvas extends HookWidget {
  ...
  final Rx<Offset> _pointerPosition = Offset(0, 0).obs;

  @override
  Widget build(BuildContext context) {
    return MouseRegion(
      cursor: drawingMode.value == DrawingMode.eraser
      ...
      child: Stack(
        children: [
          ...
          // tentativa de melhorar na hora de apagar com borracha
          ValueListenableBuilder<double>(
            valueListenable: eraserSize,
            builder: (context, size, _) {
              return _buildEraserVisual(context, size);
            },
          ),
          Listener(
            onPointerMove: (event) {
  ...
  Widget _buildEraserVisual(BuildContext context, double size) {
    if (drawingMode.value == DrawingMode.eraser) {
      return Obx(() {
        return Positioned(
          left: _pointerPosition.value.dx - size / 2,
          top: _pointerPosition.value.dy - size / 2,
          child: Container(
            width: size,
            height: size,
